...
namespace BlockchainAssignment
{
    class Block
    {
        ...
        public Block(Block lastBlock, List<Transaction> transactions, String minerAddress, String TypeTransactions)
        {
            timestamp = DateTime.Now;
            ...
            if (TypeTransactions == "greedy")
            {
                transactionList = transactionList.OrderByDescending(o => o.fee).ToList(); //Higher fee transactions completed first
            }
            else if (TypeTransactions == "random")
            {
                transactionList = transactionList.OrderBy(x => Guid.NewGuid()).ToList(); //Random order
            }
            else
            {
                transactionList = transactionList.OrderBy(o => o.timestamp).ToList(); //Longest wait first
            }
